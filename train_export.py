...
def train_and_export():
    ...
    # Sort by modification time descending to automatically grab the newest training data
    dataset_files.sort(key=lambda x: os.path.getmtime(x), reverse=True)
    dataset_path = dataset_files[0]
    print(f"Selected newest training dataset: {dataset_path}")
    ...
    archive_filename = f"checkers_valuenet_{timestamp}.onnx"
    ...
    # Save a versioned archive copy so you can track model progression
    shutil.copyfile(onnx_filename, archive_filename)
    
    print(f"Active deployment model saved to: {os.path.abspath(onnx_filename)}")
    print(f"Archived checkpoint saved to: {os.path.abspath(archive_filename)}")
    print("Ready for browser deployment and self-play iteration!")
