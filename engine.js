...
export function evaluateBoardWithNN(b) {
    if (!nnSession) return evaluateHeuristic(b);
...
export function evaluateHeuristic(b) {
    ...
    for(let r=0; r<8; r++) {
        for(let c=0; c<8; c++) {
            ...
            if(p === 1) { 
                ...
                // --- RED POSITIONAL: QUADRATIC ---
                // Rewards aggressive exponential growth as it nears promotion (row 0)
                let distAdvanced = 7 - r;
                redPos += Math.floor((distAdvanced * distAdvanced) / 4);
                ...
                // --- BLUE POSITIONAL: LINEAR ---
                // Rewards steady, proportional advancement down the board (towards row 7)
                bluePos += r * 2;
...
function quiescence(b, alpha, beta, maximizing, player, activePiece = null) {
    ...
    let standPat = useNeuralNetwork ? evaluateBoardWithNN(b) : evaluateHeuristic(b); 
