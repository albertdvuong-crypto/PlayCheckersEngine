...
self.onmessage = async function(e) {
    ...
    if (msg.cmd === 'search') {
        const { board, turn, gameHistory, thinkTime, activePiece, useNN } = msg;
        ...
        if (moves.length === 0) {
            self.postMessage({ type: 'bestmove', bestMove: null, bestVal: 0, depth: 0 });
            return;
        }
        
        // Always initialize to moves[0] so the AI is mathematically guaranteed to pick a legal move
        // or a forced capture even if evaluations encounter NaN/ties or abort immediately.
